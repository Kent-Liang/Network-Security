...
def sub_key_bytes(key_word):
    ...
    new_key_word = BitVector.BitVector(bitstring='');
    for i in range (4):
	new_key_word += sbox_lookup(key_word[8*i: 8*(i+1)])
    return new_key_word

def sub_word(temp, rconind):
    #Rotate
    temp = temp[1:] + temp[:1]
    #Apply sbox substitution
    for i in range(4):
	temp[i] = sbox_lookup(temp[i])
   
    # XOR the output of the rcon with the first part of the word
    temp[0] = temp[0] ^ BitVector.BitVector(intVal = rcon[rconind], size=8)
    return temp

def init_key_schedule(key_bvs):
    ...
    #initialize
    temp = []
    num_words = 4*(1 + rounds)
    w = init_state_array(key_bvs) + [[key_bv("00")]*4] * (num_words-4)
    #expand key
    for i in range(4,num_words):
	temp = w[i-1]
	if (i % 4 == 0):
	    temp = sub_word(temp,i/4)
	w[i] = [w[i-4][j] ^ temp[j] for j in range(4)]
    return [w[i][0] + w[i][1] + w[i][2] + w[i][3]for i in range(num_words)]
...
def add_round_key(sa, rk):
    ...
    newsa = init_state_array(BitVector.BitVector(size = 128))
    for row in range(len(sa)):
	index = 0
	for column in range(len(sa[row])):
	    newsa[row][column] =  sa[row][column] ^  rk[row][index:index+8]
	    index += 8
    return newsa
...
def sbox_lookup(input):
    ...
    row_index = int(input[0:4])
    column_index = int(input [4:8])
    hex_element = sbox[row_index][column_index]
    return BitVector.BitVector(intVal = (hex_element), size = 8)
...
def inv_sbox_lookup(input):
    ...
    row_index = int(input[0:4]);
    column_index = int(input [4:8]);
    hex_element = sboxinv[row_index][column_index];
    return BitVector.BitVector( intVal = (hex_element), size = 8);    
...
def inv_sub_bytes(sa):
    ...
    for i in range(4):
	for j in range(4):
	    sbox_value = inv_sbox_lookup(sa[i][j]);
	    sa[i][j] = sbox_value;
    return sa
...
def inv_shift_rows(sa):
    ...
    for i in range(1,4):
	row = sa[0][i] + sa[1][i] + sa[2][i] + sa[3][i]
	new_bv = shift_bytes_right(row, i)
	sa[0][i] = new_bv[0:8]
	sa[1][i] = new_bv[8:16]
	sa[2][i] = new_bv[16:24]
	sa[3][i] = new_bv[24:32]
...
def gf_mult(bv, factor):
    ...
    for i in range(8):
	#Step 1 polynomial addition
	if (bv_factor[7] == 1):
	    p^= cp_bv 
	    
	#Step 2 divides the polynomial
	bv_factor >>= 1
	bv_factor[0] = 0		    
...
def mix_col(sa,inv=False):
    '''Helper function for inverser mix columns and mix columns
    which does the matrix multiplication in this order
        # copy[0],m[0] copy[3],m[1] copy[2],m[2] copy[1],m[3]        
	# copy[1],m[0] copy[0],m[1] copy[3],m[2] copy[2],m[3]       
	# copy[2],m[0] copy[1],m[1] copy[0],m[2] copy[3],m[3]     
	# copy[3],m[0] copy[2],m[1] copy[1],m[2] copy[0],m[3]'''
    if inv: 
	m = [14,9,13,11]
    else:
	m = [2,1,1,3]
    ...
    sa[0] = gf_mult(copy[0],m[0]) ^ gf_mult(copy[3],m[1]) ^ gf_mult(copy[2],m[2]) ^ gf_mult(copy[1],m[3])
    sa[1] = gf_mult(copy[1],m[0]) ^ gf_mult(copy[0],m[1]) ^ gf_mult(copy[3],m[2]) ^ gf_mult(copy[2],m[3])
    sa[2] = gf_mult(copy[2],m[0]) ^ gf_mult(copy[1],m[1]) ^ gf_mult(copy[0],m[2]) ^ gf_mult(copy[3],m[3])	
    sa[3] = gf_mult(copy[3],m[0]) ^ gf_mult(copy[2],m[1]) ^ gf_mult(copy[1],m[2]) ^ gf_mult(copy[0],m[3]) 
...
def encrypt(hex_key, hex_plaintext):
    ...
    bv_key = key_bv(hex_key)
    bv_plaintext = key_bv(hex_plaintext)
    w = init_key_schedule(bv_key)
    sa = init_state_array(bv_plaintext)
    sa = add_round_key(sa, w[0:4])
	...
	if i != 9:
	    sa = mix_columns(sa)
	sa = add_round_key(sa, w[4*(i+1):4*(i+2)])
    return state_str(sa)	
...
def decrypt(hex_key, hex_ciphertext):
    ...
    # ADD YOUR CODE HERE - SEE LEC SLIDES 14-15
    bv_key = key_bv(hex_key)
    bv_ciphertext = key_bv(hex_ciphertext)
    w = init_key_schedule(bv_key)
    sa = init_state_array(bv_ciphertext)
    sa = add_round_key(sa, w[4*rounds:4 * (rounds + 1)])

    for i in range(rounds-1,-1,-1):
	sa = inv_shift_rows(sa)
	sa = inv_sub_bytes(sa)
	sa = add_round_key(sa, w[4*(i):4*(i+1)])
	if i != 0:
